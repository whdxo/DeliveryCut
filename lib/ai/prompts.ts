...
- 사용 가능한 도구에 "냄비"가 포함되면 menuOptions 3개 중 최소 1개는 국/찌개/탕 계열 메뉴로 작성
- 기피 재료: dislikedIngredientsText에 포함된 재료는 메뉴/재료/레시피 어디에도 절대 포함 금지
- 유통기한 임박 재료 (D-3 이하): 해당 재료를 반드시 활용하는 메뉴를 우선 추천
- 보유 재고(inventoryContext)에 있는 항목은 shoppingList에 포함하지 말 것
- menu/flavorDesign에서 실제 사용되는 품목만 shoppingList에 포함

## flavorDesign 작성 규칙
- flavorDesign.method를 반드시 확정해 작성
- menu마다 pattern TOP3를 평가하고, 점수(score 및 scores breakdown)를 채울 것
- tasteBalance는 0~5 범위 유지
- recipeStepsV2는 4~7개로 작성
- recipeStepsV2의 최소 60%는 heat 또는 timerMin 또는 why 중 하나 이상 포함
- recipeSteps는 recipeStepsV2의 text를 요약한 문자열 배열로 작성

## 출력 형식
- menuOptions: 정확히 3개, optionId는 "option-1", "option-2", "option-3"
- 메뉴 3개는 서로 다른 형태 (예: 볶음밥/덮밥/찜 등)
- steps: 각 메뉴당 최소 3줄, 최대 7줄, 간결하게
...
export function buildUserPrompt(input: GenerateInput): string {
  const { timeLimitMin, tools, ingredientsText, dislikedIngredientsText, fridgeContext, inventoryContext, recentMenus } = input
...
- ${ingredientsSection}
${inventoryContext && inventoryContext.length > 0 ? `- 보유 재고(조미료 포함): ${inventoryContext.map((i) => `${i.name} ${i.amount}${i.unit}`).join(", ")}` : ""}
${dislikedIngredientsText ? `- 기피 재료 (절대 제외): ${dislikedIngredientsText}` : ""}${recentMenusSection}

# 요청
위 조건으로:
1. 배달 대신 직접 만들 수 있는 메뉴 3가지
2. 각 메뉴의 재료, 조리 순서(3~7줄), 실패 방지 팁
3. 이 재료들로 3일간 돌려먹는 식단 플랜
4. 부족한 재료만 담은 장보기 리스트
5. flavorDesign.method, topCandidates 점수분해, recipeStepsV2를 포함해 작성
